# <code>istari <b>unlock</b></code>

## 1. About 
The `istari` executable is composed of several inter-related sub commands. Please see `istari -h` for all available options.

This part of the documentation describes options and concepts for <code>istari <b>unlock</b></code> sub command in more detail. With minimal configuration, the **`unlock`** sub command enables you to unlock a pipeline output directory. 
...
Unlocking istari pipeline output directory is fast and easy! In its most basic form, <code>istari <b>unlock</b></code> only has *one required input*.

## 2. Synopsis
```text
$ ./istari unlock [-h] --output OUTPUT
```
...
istari unlock --output /data/$USER/output
```